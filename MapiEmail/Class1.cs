Below is the source code and simple test class for creating and displaying emails. It uses the MAPI API and is therefore not subject to the same restrictions as the "mailto" shell extension.

Using the MapiMailMessage you can set the title, subject, recipients and attach files. Then show the resulting email to the user, ready from them to send. This class is very useful for supporting applications and enriching addins.

Note, that the class is a port from a VB version I had and although it does have comments, it is not up to my usual standard. I will be revisiting this and tidying it up shortly.
